...
int main() {
	...
	rep(i, n) rep(j, n) {
		int from_id = i * n + j;
		rep(to_row, n) {
			int col_sq = m - int(pow_ll(to_row - i, 2));
			if (col_sq < 0) continue;
			int diff = sqrt(col_sq);
			if (diff * diff == col_sq) {
				rep(mult, -1, 2, 2) {
					int to_col = j + mult * diff;
					if (in_range(to_col, 0, n)) {
						int to_id = to_row * n + to_col;
						g.add_edge(from_id, to_id, 1, true, 0);
					}
				}
			}
		}
	...
	rep(i, n) rep(j, n) {
		res[i][j] = ans[i * n + j];
		if (res[i][j] == g.INF) res[i][j] = -1;
	}
	foa(vec, res) vout(vec, 0);
	return 0;
}
